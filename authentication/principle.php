...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    $password = $_POST['password']; // No need to escape, as it is not used in SQL

    // Query to fetch principal credentials
    $query = "SELECT * FROM tbl_principal WHERE principal_email = '$username' LIMIT 1";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($password, $principal['principal_password'])) {
            ...
            header("Location: principle.php?error=" . urlencode($error_message));
            exit();
        }
    } else {
        $error_message = "Invalid username or password!";
...
<div class="color-splash"></div>
    <div class="login-container" id="login-container">
        <h2 class="font-weight-bold"><i class="fas fa-chalkboard-teacher"></i> Principle Login</h2>
